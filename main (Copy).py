...
'''
update_era5(variable=variable, area=area, output_path=output_path)
######## Start MLflow experiment Climate_trends #######################
mlflow.log_param("city", city)
mlflow.log_param("variable", variable)
#mlflow.log_param("start_year", config["start_year"])
#mlflow.log_param("end_year", config["end_year"])
...
#### Log the start and end date of the data ####
...
###### Genearte the EDA plots and save to output folder #####
...
#####################
###### Log the Bayesian Average trends #########################
mlflow.log_metric(
    "average_warming_trend",
    float(avg_trend)
)
...
mlflow.log_artifact(
    dir_name + run_name + "posterior_values.csv",
    artifact_path="bayesian"
)

mlflow.log_artifact(
    dir_name + run_name_max + "posterior_values.csv",
    artifact_path="bayesian"
)
##############################################################
